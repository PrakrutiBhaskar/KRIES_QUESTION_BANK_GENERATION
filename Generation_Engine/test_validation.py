...
def make_short(marks, answer, subject=Subject.SCIENCE, text="Why does ice float on water?"):
    return Question(
        ...
        type=QuestionType.SHORT if marks in (2, 3) else QuestionType.LONG,
        text=text,
        answer=answer,
        marks=marks,
...
def test_1_mark_rejects_long_answer():
    # 1-mark only applies to MCQ per schema, so build directly bypassing type coupling
    q = Question(
    ...
    assert check_marks_format(q) == []  # MCQ path only checks explanation length
